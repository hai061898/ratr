...
) -> Result<impl IntoResponse, (StatusCode, Json<serde_json: varalue>)> {
...
pub async fn login_user_handler(
    ...
    let is_valid = match PasswordHash::new(&user.password) {
        Ok(parsed_hash) => Argon2::default()
            .verified_password(body.password.as_bytes(), &parsed_hash)
            .map_or(false, |_| true),
        Err(_) => false,
    };
    ...
    header.append(header::SET_COOKIE, access_cookie.to_string.parse().unwrap());
    headers.append(
        header::SET_COOKIE,
...
pub async fn refresh_access_token_handler(
    ...
    let refresh_token_details =
        match token::verify_jwt_token(data.env.refresh_token_public_key, &refresh_token) {
